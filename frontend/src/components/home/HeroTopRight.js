import "./heroTopRight.css";

function HeroTopRight() {
  return (
    <>
      <div className="container hero-top-right-container">
        <div className="card hero-top-right-card">
          <div className="card-body hero-top-right-card-body">
            <h1 className="card-title hero-top-right-card-title mb-4">
              Why choose us?
            </h1>

            <p className="card-text hero-top-right-card-text mb-2">
              Support, advice, information and the necessary representation.
            </p>

            <p className="card-text hero-top-right-card-text mb-2">
              Are you looking for a visa to enter, live, travel, work and invest
              in Ireland?
            </p>

            <p className="card-text hero-top-right-card-text mb-2">
              We are immigration consultants and assist our clients in
              submitting visa applications of the highest quality.
            </p>

            <p className="card-text hero-top-right-card-text mb-2">
              Call us on (+353) 874797439 for immediate support and advice with
              your immigration case. We can speak over the phone or by zoom and
              google meet.
            </p>

            <p className="card-text hero-top-right-card-text mb-2">
              What'sApp: +353874797439 https://wa.me/message/HAIECD27SCPOB1
              info@equalityimmigration.ie
            </p>

            <p className="card-text hero-top-right-card-text mb-2">
              We prioritize customer satisfaction above all else, striving to
              make every journey an extraordinary experience.
            </p>
          </div>
        </div>
      </div>
    </>
  );
}

export default HeroTopRight;
